...
def main():
    parser = argparse.ArgumentParser(
        description="Downloads 3RScan public data release."
    )
    parser.add_argument(
    ...
    if args.id:  # download single scan
        ...
        if len(file_types) == len(FILETYPES):
            print(
                "WARNING: You are downloading the entire 3RScan release which requires "
                + RELEASE_SIZE
                + " of space."
            )
        else:
            print(
                "WARNING: You are downloading all 3RScan scans of type " + file_types[0]
            )
        print(
