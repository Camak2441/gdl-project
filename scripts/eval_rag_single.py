...
from encoders import get_query_encoder
from models import (
...
def make_gnn_search_tool(k: int) -> dict:
    return {
        "type": "function",
        "function": {
            "name": "search_scene_graph",
            "description": (
                f"Search the scene graph for objects relevant to a query. "
                f"Returns the top {k} most relevant objects and their relationships."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "query": {
                        "type": "string",
                        "description": (
                            "A simple natural language question about what is in the scene graph. "
                            "Ensure it is phrased as a question. "
                            'Example queries: "What is a bed?" or "What to the right of the bed might I need to be careful of?" '
                        ),
                    }
                },
                "required": ["query"],
...
def preprocess_scene_graph(sg_json: dict, selected_ids: set | None = None) -> dict:
    ...
    Relationships are included when one of the endpoints are in selected_ids.
    """
...
def ask_llm_with_tool(
    ...
    """Single conversation: LLM calls the GNN as a tool, then answers.

    Turn 1 — LLM is forced to call search_scene_graph(query=...).
    The query text is encoded and run through the GNN; top-k nodes are
    returned as the tool result.
    Turn 2 — LLM answers the original question from the retrieved nodes.
    """
    messages = [
    ...
    # Turn 1: force the LLM to call the GNN tool
    gnn_tool = make_gnn_search_tool(k)
    r1 = client.create_completion(
        messages,
        tools=[gnn_tool],
        tool_choice={"type": "function", "function": {"name": "search_scene_graph"}},
    )
    msg1 = r1.choices[0].message

    if not msg1.tool_calls:
        # Model didn't call the tool; fall back to direct answer
        return extract_answer_ids(msg1.content or "") or []
    ...
    encoded_query = query_encoder([gnn_query_text])  # [1, query_dim]
    scores = run_gnn_for_scan(gnn_model, scan_data, encoded_query, device)

    n_nodes = scores.shape[1]
    top_k = min(k, n_nodes)
    top_k_indices = scores[0].topk(top_k).indices.tolist()
    selected_ids = {str(node_map[idx]) for idx in top_k_indices}

    reduced_sg = preprocess_scene_graph(sg_json, selected_ids)
    logger.info(
        "GNN returned %d nodes: %s",
        len(reduced_sg["objects"]),
        [(o.get("id"), o.get("label")) for o in reduced_sg["objects"]],
    )
    tool_content = json.dumps(reduced_sg, indent=2)

    # Append assistant turn (with tool call) and tool result
    messages.append(
        {
            "role": "assistant",
    ...
    # Turn 2: LLM answers the original question from retrieved nodes
    r2 = client.create_completion(messages)
    ids = extract_answer_ids(r2.choices[0].message.content or "")
...
def main():
    ...
    print(f"Total questions to process: {total_remaining} (2 LLM calls each)")
    ...
    query_encoder = get_query_encoder(QUERY_ENCODER)
    ...
    for split in splits:
        ...
        for qf in question_files:
            ...
            for i, question_text in enumerate(question_texts):
                ...
                logger.info(
                    "CALL  [%d/%d] scan=%s q=%d question=%r",
                    call_num,
                    split_total,
                    scan_id,
                    i,
